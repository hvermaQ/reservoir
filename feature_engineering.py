...
def create_lagged_binary_features(
    ...
    """
    Perform lagged context binarization and produce lagged feature windows in one step.

    For each t >= lag_window-1:
      - Use values[t-lag_window+1 : t+1] to compute local mean and std.
      - Binarize current value into 4 discrete labels {0,1,2,3} using thresholds:
          0: x_t < μ - 2σ (very negative)
          1: μ - 2σ <= x_t < μ - σ (small negative)
          2: μ - σ <= x_t < μ + σ (small positive)
          3: x_t >= μ + 2σ (very positive)
      - Construct feature vector as lagged window of these binarized labels.
    ...
    lag_window : int
        Length of lag window.
    sigma_threshold : float
        Inner threshold multiplier in sigma units (default 1.0).
    two_sigma_threshold : float
        Outer threshold multiplier (default 2.0).

    Returns
    -------
    X : np.ndarray, shape (T-lag_window+1, lag_window)
        Lagged feature matrix of binarized labels.
    y : np.ndarray, shape (T-lag_window+1,)
    ...
    for t in range(T):
        ...
        pos_large = mu_t + two_sigma_threshold * sigma_t
...
def extract_sigmaz_reset_with_washout(result, n_steps, washout_length=10):
    ...
    n_steps : int
        Number of timesteps to extract (including washout).
    washout_length : int
        Number of initial timesteps to discard.

    Returns
    -------
    sigmaz : np.ndarray, shape (n_steps - washout_length,)
    ...
    if total_prob == 0:
        return np.ones(n_steps - washout_length)
